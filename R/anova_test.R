...
#'  computed from the noncentral F distribution (Steiger, 2004) in base R, and
#'  matches \code{effectsize::eta_squared(partial = TRUE, ci = )}. No interval is
#'  provided for generalized eta squared (\code{"ges"}), which has no standard
#'  closed-form interval. Default is \code{NULL} (no interval; output unchanged).
