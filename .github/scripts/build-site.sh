...
# search.json where the site builds one. That happened here: the published
# sitemap.xml advertised https://rpkgs.datanovia.com/rstatix/CLAUDE.html until
# 2026-08-02, because the old `make site` deleted docs/CLAUDE.html and nothing
# else -- the page 404'd while the index kept pointing at it.
#
...
#      deployed.
#
# This script lives in .github/scripts/ on purpose: a tools/ directory would be
# copied into the published site by pkgdown, .github/ is not. Both this file and
# the Makefile are in .Rbuildignore, so neither reaches the CRAN tarball.
#
# Usage:
#   .github/scripts/build-site.sh              # build, scrub, verify
#   .github/scripts/build-site.sh --scrub-only # scrub + verify an existing docs/
#
# RSTUDIO_PANDOC must point at the DIRECTORY holding the pandoc binary; pkgdown
# needs it to render the vignette and the articles.
set -euo pipefail
...
pkgdown will render it into a PUBLIC page and list it in sitemap.xml (and index
its full text in search.json where one is built). Project instructions belong in
.claude/CLAUDE.md, which pkgdown cannot see: it globs only the package root and
.github/.
...
if [ "$SCRUB_ONLY" -eq 0 ]; then
  RSTUDIO_PANDOC="${RSTUDIO_PANDOC:-/Applications/RStudio.app/Contents/MacOS/pandoc}"
  export RSTUDIO_PANDOC
  export PATH="$RSTUDIO_PANDOC:$PATH"
  command -v pandoc >/dev/null 2>&1 || {
    echo "ERROR: pandoc not found. Set RSTUDIO_PANDOC to the directory holding it." >&2
    exit 1
  }
...
# case-sensitively against "issue_template.md", so an uppercase file renders.
...
# leaves the entire document publicly searchable. rstatix's current site ships
# no search.json, but pkgdown adds one as soon as search is enabled -- this runs
# unconditionally so the guard does not depend on today's configuration.
...
echo "Deploy with a MIRRORING sync (rsync --delete) after diffing against the live"
echo "tree. A plain re-upload leaves an already-published CLAUDE.html and a stale"
echo "sitemap.xml in place on the server."
