...
- The declared minimum version of dplyr is now 1.0.0, matching what the package has required since it began importing `all_of()` and `any_of()`. `NAMESPACE` imports both, dplyr re-exports them from 1.0.0 onwards, and `R/anova_summary.R` calls them, so an older dplyr failed on the import rather than at a call site. The declared minimum still said 0.7.1.

## Bug fixes

- `add_cld()` no longer reads a comparison it does not have as "not significantly different" (#323). A shared letter is a statistical conclusion, and it was being drawn for pairs that were never tested. `games_howell_test()` returns `p.adj = NA` for a comparison whose Welch standard error is zero or undefined — two groups that are both constant, or a group with a single observation — and those groups were then given the same letter: for group means of 0, 10 and 20 with the first two constant, `add_cld()` labelled them both `a`, although every Tukey p-value on that data is below 1e-6. The same held for an input that simply lacks the comparison, such as a `ref.group` result, where the non-reference groups shared a letter on the strength of a pair that was never compared. A group involved in a comparison of unknown significance now gets `cld = NA` and a warning naming the pairs. A group keeps its letter only when every one of its comparisons is present *and* carries a p-value, so an input where all pairs are established is unchanged. The rule is symmetric rather than minimal: a single group whose comparisons cannot be estimated removes every group it is compared against, so a display can come back entirely empty even where the other groups were compared successfully with each other — the warning names the pairs responsible.

- `get_test_label()` no longer fills its p slot from a neighbouring column when it cannot settle on a p-value column (#339). When the result carried none, the slot took the `parameter` column and the label read `Anova, p = 2,57` — the degrees of freedom — for a test whose p was 9.53e-16. An `anova_test()` result is a base data frame rather than a tibble, so `$p` partial-matched `parameter`; a tibble-based result left the p slot empty instead. The same happened when several p-value columns were present and none was named — `p.adj` and `p.value` without a plain `p`, say — since detection then had no single column to take. All of these now stop, naming the columns the result does have. Separately, `p.col` naming a column the result does not have used to fall back to auto-detection, so asking for `p.col = "p.adj"` on a table with only `p` silently returned the unadjusted p-value; that is now an error too. Auto-detection is unchanged where it applies: the default `p.col = "p"` still finds `p.adj` or `p.value` when `p` itself is absent. A `p.col` naming a column that is present is still taken at your word, whether or not it holds a p-value: `p.col = "ges"` labels the effect size as the p-value, as it did before.

- `df_split_by()`, `df_label_both()` and `df_label_value()` no longer attach another group's label to a subset (#324). The label was built by sorting the grouping variables and then writing the sorted labels back onto the unsorted rows, so any group whose position differed from its sorted rank got its neighbour's label — the wrong label also went into each nested subset. For example, `df_split_by(df, vars = "region")` on a character `region` running North, East, South returned North's data labelled `region:East`. Labels are now built row by row from each group's own values. `df_label_both()` was affected even when the grouping column was a factor, since the variable name is pasted onto the levels before the sort. The label's factor levels still follow the sorted row order (see the next entry for a change to how that order is derived), and `df_unite_factors()` itself is untouched.

- `df_label_both()` now takes its factor level order from the grouping variables themselves rather than from the pasted `"<variable>:<value>"` strings (#326). Sorting the strings ordered a numeric variable lexicographically and ignored a factor's own levels, so `df_label_both()` and `df_label_value()` disagreed on the same data. A `dose` column of 0.5, 1, 2, 10 gave the levels `dose:0.5`, `dose:1`, `dose:10`, `dose:2` and now gives `dose:0.5`, `dose:1`, `dose:2`, `dose:10`; a factor with levels North, East, South gave `g:East`, `g:North`, `g:South` and now follows the declared order. A missing value sorts last instead of alphabetically as the string `"g:NA"`, and a zero-row input no longer gains a phantom `"g:"` level. **This changes panel ordering** wherever a byte-order sort of the pasted strings disagrees with sorting the values, which cuts across column types rather than following them. It moves, for example: a numeric column whose values do not sort as text (`dose:10` before `dose:2`); a factor whose declared level order differs from the byte order of its labels — mixed case is one common way to reach that, since the string sort put every uppercase initial first, but it is the order that decides, not the case (levels `c("control", "Treatment")` and `c("banana", "Apple")` move; `c("Control", "Drug")` and `c("Apple", "banana")` do not); and a column with a missing value, but only where some present value sorted after the literal text `NA` — `OJ`/`VC` and lowercase names move, whereas `c("Control", "Drug", NA)` already had the missing group last and keeps it there. Where the two orders agree, as for a character column of uppercase-initial names, nothing changes. `df_label_value()` and `df_unite_factors()` keep the level order they already had: both sorted the grouping columns themselves, so `df_label_both()` now agrees with them rather than diverging. To restore a previous panel order, set the levels on the label column after labelling.

- `df_label_both()` and `df_label_value()` no longer read the label back out of the data instead of writing it (#326). The label was assigned with `mutate()` using a bare `label` symbol, which dplyr resolves against the data first, so a data frame carrying a column named `label` had that column's values written into `label_col`, and a grouping variable named `label` made the labeller a no-op that returned the data untouched. Two related changes came out of assigning the label properly. A data frame grouped with `dplyr::group_by()` or `dplyr::rowwise()` is now labelled — with the same values and levels as the same data ungrouped — where it previously raised a size error, or, when the grouping variable is not itself one of two or more labelling variables, a recycling error from the grouping column that `dplyr::select()` adds back. On an ungrouped frame, labelling with no grouping variable at all is unchanged from 1.1.0: `df_label_value()` returns an empty label, and `df_label_both()` cannot build one, so dplyr rejects the zero-length result on a frame with rows. The one difference on those inputs is that neither now depends on whether the data happens to carry a `label` column for the old code to echo back. On a grouped frame, `df_label_value()` follows the change described above and returns that empty label where 1.1.0 raised a size error; `df_label_both()` still cannot build a label, and errors there as it did before.


- `welch_anova_test()` now reports the number of observations analysed rather than the number of rows supplied (#334). `stats::oneway.test()` drops a row whose value for any term of the formula is missing, so the reported `n` was inflated: for `ToothGrowth` with 20 missing outcomes it read `n = 60` where 40 rows were usable, and a missing group cost a row the same way with the outcome intact. It now reads 40, agreeing with `anova_test()` and `kruskal_test()` on that data, which report it that way already. Where every supplied row is usable, the count is unchanged. The count is taken from the formula's variables without evaluating the formula, so one case is deliberately not covered: a row lost to a non-finite *transformed* value, as in `log(len) ~ dose` with a non-positive `len`, is still counted, because detecting it would mean evaluating the transformation a second time and that would change the statistic itself.

- `get_test_label()` and `get_n()` now report the number of observations a between-subjects ANOVA analysed rather than the number of rows supplied, for a test fitted from a formula or from a model the caller supplied (#322). The `dv`/`between` interface already reported the analysed count. `anova_test()` fits the model on the rows the fit can use, so data with a missing outcome or model variable gave a label whose `n` contradicted the degrees of freedom printed beside it: `F(2,37) = 34.97 [...] n = 60` where only 40 rows were usable. It now reads `n = 40`, agreeing with `kruskal_test()` on the same data, which carried its own `n` and was already right. `get_n()` also returns the analysed count for a model the caller fitted (`anova_test(lm(len ~ dose, data = df))`), where it returned `NA`; the detailed label for such an object consequently carries an `n` field where it previously printed none, complete data included. A missing value is not the only way to lose a row: a formula transformation that yields a non-finite value drops rows without any `NA` in the data, and the count now follows the fit there too — `anova_test(df, len ~ log(x))` with `x` negative in five rows reports `n = 55` beside `F(1,53)`, where it reported 60. Wherever the fit used every supplied row, a data-frame call's count is unchanged — model input is the one shape that moves with nothing dropped — as is the APA style, which does not print `n`. Two counts are unchanged and can still exceed the number analysed: a repeated-measures or mixed design counts the subjects left after rows with missing values are dropped, so a subject missing only some of its within-cells is counted although the model drops it (#333), and grouped input (`group_by() %>% anova_test()`) counts the rows supplied (#331).

- Printing an `anova_test()` result no longer fails after an operation that does not carry the stashed arguments through (#336). `res[, c("DFn", "DFd")]`, `dplyr::select()` and `dplyr::mutate()` keep the `anova_test` class but not those arguments, so `print()` had no sum of squares type to name and stopped with `EXPR must be a length 1 vector`. Such a result now prints under a plain `ANOVA Table` header. A result whose arguments still carry a type is unchanged, header included; `res[1, ]`, `head()`, `dplyr::filter()`, `dplyr::arrange()` and `dplyr::slice()` keep them, whereas `dplyr::relocate()`, `dplyr::distinct()`, `dplyr::transmute()` and `subset()` do not — dropping a column is not what decides it. `get_test_label()` and `plot()` still fail on such a result, for the same reason and unchanged here.
