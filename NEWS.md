...
- The declared minimum version of dplyr is now 1.0.0, which the package has in fact required since it began importing `all_of()` and `any_of()`: dplyr re-exports both from 1.0.0 onwards, so an older dplyr failed on the `NAMESPACE` import rather than at a call site. The declared minimum still said 0.7.1.

## Bug fixes

### Panel labelling and splitting

- `df_split_by()`, `df_label_both()` and `df_label_value()` no longer attach another group's label to a subset (#324). Labels were built by sorting the grouping variables and writing the sorted labels back onto the unsorted rows, so any group whose position differed from its sorted rank got its neighbour's label — in the data as well as in the nested subset. `df_split_by(df, vars = "region")` on a character `region` running North, East, South returned North's data labelled `region:East`. Labels are now built row by row from each group's own values. Factor columns were affected too, since `df_label_both()` pastes the variable name onto the levels before sorting. `df_unite_factors()` is untouched.

- `df_label_both()` now takes its factor level order from the grouping variables rather than from the pasted `"<variable>:<value>"` strings (#326). Sorting the strings ordered a numeric variable lexicographically and ignored a factor's declared levels: `dose` of 0.5, 1, 2, 10 gave `dose:0.5`, `dose:1`, `dose:10`, `dose:2` and now gives them in numeric order; a factor with levels North, East, South now follows the declared order. A missing value sorts last rather than alphabetically as `"g:NA"`, and a zero-row input no longer gains a phantom `"g:"` level. **This changes panel ordering** wherever a byte-order sort of the pasted strings disagreed with sorting the values — numeric columns, factors whose declared order differs from the byte order of their labels (mixed case is one common route), and columns with a missing value. Where the two orders agree nothing moves. `df_label_value()` and `df_unite_factors()` keep the order they had, so `df_label_both()` now agrees with them. To restore a previous order, set the levels on the label column after labelling.

- `df_label_both()` and `df_label_value()` no longer read the label back out of the data instead of writing it (#326). The label was assigned with a bare `label` symbol, which dplyr resolves against the data first, so a data frame carrying a `label` column had that column written into `label_col`, and a grouping variable named `label` made the labeller a no-op. Assigning it properly also means a frame grouped with `dplyr::group_by()` or `dplyr::rowwise()` is now labelled — same values and levels as the same data ungrouped — where it previously raised a size or recycling error. Labelling with no grouping variable at all is unchanged from 1.1.0 on an ungrouped frame: `df_label_value()` returns an empty label and `df_label_both()` cannot build one. Neither now depends on whether the data happens to carry a `label` column.

### Test labels

- `get_test_label()` and `get_n()` now report the number of observations a between-subjects ANOVA analysed rather than the number of rows supplied, for a test fitted from a formula or from a caller-supplied model (#322). The `dv`/`between` interface already reported the analysed count. A label read `F(2,37) = 34.97 [...] n = 60` where only 40 rows were usable; it now reads `n = 40`, agreeing with `kruskal_test()` on the same data. `get_n()` also returns a count for `anova_test(lm(...))`, where it returned `NA`, so that label gains an `n` field it did not print before, complete data included. A missing value is not the only way to lose a row: `anova_test(df, len ~ log(x))` with `x` negative in five rows now reports `n = 55` beside `F(1,53)`, where it reported 60. The APA style is unaffected — it does not print `n`. Two counts are unchanged and can still exceed the number analysed: a repeated-measures or mixed design counts subjects surviving row removal (#333), and grouped input counts the rows supplied (#331).

- `get_test_label()` no longer fills its p slot from a neighbouring column when it cannot settle on a p-value column (#339). With no such column the slot took `parameter`, so the label read `Anova, p = 2,57` — the degrees of freedom — for a test whose p was 9.53e-16. (An `anova_test()` result is a base data frame, so `$p` partial-matched; tibble-based results left the slot empty.) The same happened when several p-value columns were present and none was named, `p.adj` and `p.value` without a plain `p`. All of these now stop, naming the columns the result has. A `p.col` naming a column the result does not have is now an error too, rather than silently falling back — asking for `p.col = "p.adj"` on a table with only `p` returned the unadjusted p-value. Auto-detection is unchanged where it applies: the default still finds `p.adj` or `p.value` when `p` is absent. A `p.col` naming a column that *is* present is still taken at your word, whether or not it holds a p-value: `p.col = "ges"` labels the effect size as the p-value, as it did before.

### Compact letter display

- `add_cld()` no longer reads a comparison it does not have as "not significantly different" (#323). A shared letter is a statistical conclusion, and it was being drawn for pairs that were never tested. `games_howell_test()` returns `p.adj = NA` when a pair's Welch standard error is zero or undefined — both groups constant, or one with a single observation — and those groups were then given the same letter: for group means of 0, 10 and 20 with the first two constant, both were labelled `a`, although every Tukey p-value on that data is below 1e-6. An input that simply lacks the comparison, such as a `ref.group` result, behaved the same way. A group involved in a comparison of unknown significance now gets `cld = NA` and a warning naming the pairs, and keeps its letter only when every one of its comparisons is present *and* carries a p-value. The rule is symmetric rather than minimal, so one group whose comparisons cannot be estimated removes every group it is compared against and the display can come back empty — the warning names the pairs responsible.

### Printing

- Printing an `anova_test()` result no longer fails after an operation that does not carry the stashed arguments through (#336). `res[, c("DFn", "DFd")]`, `dplyr::select()` and `dplyr::mutate()` keep the `anova_test` class but not those arguments, so `print()` had no sum of squares type to name and stopped with `EXPR must be a length 1 vector`. Such a result now prints under a plain `ANOVA Table` header; one that still carries a type is unchanged, header included. Dropping a column is not what decides it: `res[1, ]`, `head()`, `filter()`, `arrange()` and `slice()` keep the arguments, whereas `relocate()`, `distinct()`, `transmute()` and `subset()` do not. `get_test_label()` and `plot()` still fail on such a result, for the same reason and unchanged here.
