...
- `tidy()` and `glance()` methods for the `rstatix_test` objects returned by the test functions (`t_test()`, `wilcox_test()`, `anova_test()`, `kruskal_test()`, and the rest). The results are already tidy tibbles; `tidy()` drops the internal `rstatix` classes and the stashed test arguments so the object passes cleanly to tools that dispatch on `generics::tidy()` / `generics::glance()`, such as `broom`, `gtsummary` and `gt`. `glance()` returns a one-row summary with the test `method` and `n`, the number of comparisons or model terms. Previously `tidy()` on such an object fell through to `broom`'s deprecated data-frame tidier and `glance()` errored.
